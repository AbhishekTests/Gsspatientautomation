...
describe('All Prescriptions test cases', function () {
  ...
  it.skip('Prescription Creation with New Symptom Not Present in Dropdown', () => {
    // const testname = 'test';
  ...
  it('edit prescription', () => {
    ...
    let FPValue = faker.number.int({ min: 1, max: 20 }).toString();
    cy.verifyUserRoleVisible('Doctor')
    cy.get('[class="item-name"]').eq(12).click()
    cy.headingstyle('Prescription')
    cy.get('[class="ant-tag ant-tag-blue css-1drr2mu"]').eq(1).click()

    cy.get('.ant-form-item-control-input').eq(0).click()
    cy.get('[class="anticon anticon-plus"]').click()
    cy.dropdown(28, 1)
    cy.get('.ant-form-item-control-input').eq(29).type(FPValue)
    cy.get('[class="ant-btn css-1drr2mu ant-btn-primary"]').eq(5).click()

    cy.dropdown(0, 0)
    cy.get('#Patient_Prescription_advice').clear().type('Take Medicine On Time')
    cy.get('[class="ant-btn css-1drr2mu ant-btn-primary"]').eq(3).click()
    cy.wait(2000)
