...
describe('Patient role', () => {
  ...
  it('Create New Patients and Verify Successful Creation', function () {
    ...
    cy.get(initialLength).then(($el) => {
      ...
      cy.wait(3000)
      cy.dropdown(13, 0) 
      cy.wait(2000)    // Handle State dropdown
      cy.get('#dateOfBirth').type("04-05-2000")
  ...
  it('Verify user registration count no. in this month', () => {
    cy.verifyUserRoleVisible('Admin')
    ...
    cy.get('.ant-select-dropdown:not(.ant-select-dropdown-hidden)').should('be.visible')
      .find('[class="ant-select-item-option-content"]').eq(2).click({ force: true }).wait(20000)
  ...
  it('verify user appointment count no. in this month', () => {
    cy.verifyUserRoleVisible('Admin')
    cy.get('[class="item-name"]').eq(15).click()
    ...
    cy.get('[class="ant-select-selector"]').eq(0).click()
    cy.get('.ant-select-dropdown:not(.ant-select-dropdown-hidden)').should('be.visible')
      .find('[class="ant-select-item-option-content"]').eq(2).click({ force: true }).wait(2000)
