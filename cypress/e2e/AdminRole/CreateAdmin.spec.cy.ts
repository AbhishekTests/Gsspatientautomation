...
describe('Admin Role Management Suite', () => {
  ...
  it('Update Existing Admin Information and Validate Changes', function () {
    ...
    cy.get('[class="ant-btn css-1drr2mu ant-btn-primary"]').should('have.text', 'Update')
    ...
    cy.get('#dateOfBirth').clear().type("2001-09-01")
    cy.get('#address').clear().type(adrees);

    cy.get('[class="ant-btn css-1drr2mu ant-btn-primary"]').click()
    cy.get('[class="ant-message-custom-content ant-message-success"]>span').eq(1).should('have.text', 'Admin Updated Successfully')
